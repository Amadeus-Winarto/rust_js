...
import { WorkerId, JOIN_THREAD_EXITED, JOIN_THREAD_NOT_EXITED, LOCK_ACQUIRED, LOCK_NOT_ACQUIRED } from './worker'
  ...
  port.onmessage = (message) => {
    ...
    switch (thread.status) {
      ...
      default:
        ...
        throw Error("VM received a thread from worker that is still in RUNNING status")
    }
...
function INITIALIZE(heapSize: number, p: Program, numOfWorkers: number) {
  ...
  for (let i = 0; i < numOfWorkers; i++) {
    ...
    worker.postMessage([HEAP, FREE, p], [threadChannel.port2, syscallChannel.port2])
...
export function runWithProgram(p: Program, heapSize: number, numOfWorkers: number) {
