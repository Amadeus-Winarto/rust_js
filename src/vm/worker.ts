...
function HEAP_SET_BYTE_AT_OFFSET(
  address: number,
  offset: number,
  value: number,
): void {
...
function HEAP_SET_2_BYTES_AT_OFFSET(
  address: number,
  offset: number,
  value: number,
): void {
...
M[OpCodes.ADDG] = () => {
  ...
  if (IS_NUMBER(I) && IS_NUMBER(G)) {
    ...
    throw Error(
      "Not both arguments to add.g are of type number or type string",
    );
  }
  OS.push(RES);
  PC = PC + 1;
...
M[OpCodes.EQG] = () => {
  ...
  if (IS_NUMBER(I) && IS_NUMBER(G)) {
    ...
    throw Error(
      "Not both arguments to eq.g are of type number or type string or type bool",
    );
  }
  NEW_BOOL();
  OS.push(RES);
...
M[OpCodes.NEQG] = () => {
  ...
  if (IS_NUMBER(I) && IS_NUMBER(G)) {
    ...
    throw Error(
      "Not both arguments to neq.g are of type number or type string or type bool",
    );
  }
  NEW_BOOL();
  OS.push(RES);
...
self.onmessage = (message) => {
  ...
  INITIALIZE(
    data[HEAP_DATAVIEW_IDX],
    data[FREE_UINT32_ARR_IDX],
    data[PROGRAM_IDX],
