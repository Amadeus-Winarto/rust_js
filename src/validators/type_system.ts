  ...
  visitFunction_declaration(
    ...
    if (maybe_parameter_list === undefined) {
      return {
        ok: false,
        error: new Error(
          `Line ${ctx.start.line}: function '${name}' has no parameters. This is a SyntaxError!`,
        ),
      };
    }
    ...
    if (return_statements.length === 0 && return_type !== "()") {
      return {
        ok: false,
        error: new Error(
          `Line ${ctx.start.line}: function '${name}' expects a return statement but none found. There's a bug in the TypeValidator!`,
        ),
      };
    }
    ...
    for (const return_statement of return_statements) {
      ...
      if (
        return_type_tag !== actual_return_type.value.type &&
        !is_promotable(actual_return_type.value.type, return_type_tag)
      ) {
        return {
          ok: false,
          error: new Error(
            `Line ${ctx.start.line}: function '${name}' expects return type ${return_type} but got ${actual_return_type.value.type}`,
          ),
        };
      }
    }
